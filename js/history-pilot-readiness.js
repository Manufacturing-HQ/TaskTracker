...
(() => {
  ...
  function buildPanel(){
    ...
    panel.innerHTML=`<div class="row"><div><h2 style="margin:0">Transaction Time Corrections</h2><div class="tx-note">Supervisor/Manager/Admin only. Every correction requires a reason and is written to the correction audit log.</div></div></div>
      <div class="tx-toolbar">
        <button id="tx-load" class="ghost" type="button">Refresh Transactions</button>
      </div>
      <div class="tx-note">Uses the Employee, Start Date, and End Date selected in Employee History above. Clicking Load History refreshes both sections.</div>
  ...
  async function loadTransactions(){
    ...
    try{
      ...
      if(!employeeId||!startDate||!endDate){host.innerHTML='<div class="empty">Select an employee and date range above, then click Load History.</div>';return;}
  ...
  function openEditor(r){
    ...
    if(r.is_open){
      ...
      bg.querySelector("#tx-save").onclick=async()=>{
        ...
        try{await rpc("force_stop_history_session_local",{p_session_token:token(),p_job_id:r.job_id,p_forced_ended_local:ended,p_correction_reason:reason,p_comments:bg.querySelector("#tx-comments").value||null});bg.remove();document.getElementById("employee-load")?.click();}catch(e){modalMessage(bg,e.message);}
      };
    }else{
      ...
      bg.querySelector("#tx-save").onclick=async()=>{
        ...
        try{await rpc("correct_history_session_time_local",{p_session_token:token(),p_job_session_id:r.job_session_id,p_new_started_local:started,p_new_ended_local:ended,p_correction_reason:reason});bg.remove();document.getElementById("employee-load")?.click();}catch(e){modalMessage(bg,e.message);}
      };
    }
  }
