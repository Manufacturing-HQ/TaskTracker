...
(() => {
  ...
  async function login(event) {
    ...
    if (!["Supervisor", "Manager", "Administrator"].includes(role)) {
      setMessage("This pilot is for Supervisor, Manager, and Administrator accounts.", "error");
      return;
    }
  ...
  function setView(view) {
    const labels = {
      home: ["Home / Live Status", "Current employee status and team performance."],
      overview: ["Supervisor Operations", "Overview of daily supervisor workflows."],
      ...
      queue: ["Task Queue", "Pending and completed Supervisor Operations tasks."]
    };
  ...
  async function loadQueue() {
    ...
    (tasks || []).forEach((t) => {
      ...
      if (t.status === "Pending") {
        const b = document.createElement("button"); b.className = "ghost"; b.textContent = "Start"; b.onclick = async () => { await rpc("start_supervisor_task", { p_session_token: sessionToken, p_supervisor_task_id: t.supervisor_task_id }); await loadQueue(); }; actions.appendChild(b);
      }
      if (["Pending", "In Progress"].includes(t.status)) {
        const b = document.createElement("button"); b.className = "primary"; b.textContent = "Complete"; b.style.marginLeft = "8px"; b.onclick = async () => { await rpc("complete_supervisor_task", { p_session_token: sessionToken, p_supervisor_task_id: t.supervisor_task_id, p_completion_notes: null }); await loadQueue(); }; actions.appendChild(b);
      }
      wrap.appendChild(div);
    });
    if (!wrap.children.length) wrap.innerHTML = '<div style="color:#64748b">No Supervisor Operations tasks match this view.</div>';
  }
